 ...
 *
 * It matters concretely here: below `SCHEMA_VERSION` the `api_keys_slow_kdf_only` and
 * `quota_windows_within_limit` CHECKs may not exist, and a service that could create them at boot
 * is a service that could start without them. `index.ts` asserts the version and refuses to serve.
 *
